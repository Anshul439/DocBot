...
const SignInPrompt = ({ onClose, onSignIn }: { onClose: () => void; onSignIn?: () => void }) => {
  ...
  return (
    ...
    >
      <div
        className={`bg-[#1A1A1A] rounded-lg p-4 max-w-xs w-full mx-4 transform transition-all duration-300 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
          }`}
      >
        ...
        <div className="flex flex-col gap-2">
          <button
            ...
            className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2 px-4 rounded-sm transition-colors text-sm"
          >
            Sign In
          </button>
